...
```text
从零做一个可运行的浏览器贪吃蛇游戏，要求视觉精美、有 README、有测试、有日志和最终报告
```

也可以直接用完整命令启动：

```cmd
cbduel.cmd run "从零做一个可运行的浏览器贪吃蛇游戏，要求视觉精美、有 README、有测试、有日志和最终报告" --mode tui --rounds 8 --min-rounds 3 --time-limit-min 120 --min-runtime-min 30 --landing soft --refresh-interval-sec 5 --auto-commit true --branch true
```
...
```cmd
cbduel.cmd run "制作一个想象力不设限的贪吃蛇游戏，持续扩展玩法、视觉、音效、关卡、成长系统、AI敌人、地图机制、剧情、技能、道具、多人模式、物理效果和隐藏彩蛋。每轮输出本轮实现、下一步创意和下一轮计划。" --mode tui --rounds 50 --min-rounds 50 --time-limit-min 480 --min-runtime-min 420 --landing soft --refresh-interval-sec 5 --auto-commit true --branch true
```
